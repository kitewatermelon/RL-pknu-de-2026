# 프로그램 3-6 FrozenLake 과업을 위한 가치 반복 알고리즘(의도한 방향의 확률을 높인 스ᄐ
    ...
    for action in range(env.action_space.n):
        if len(env.P[state][action])==3:
            for i,transition in enumerate(env.unwrapped.P[state][action]):
                env.P[state][action][i]=(prob[i],transition[1],transition[2],transition[3])
